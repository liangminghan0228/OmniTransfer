...
def get_bf_by_machine_threhold(calc_latency, score_type='g'):
    ...
    for machine_id in tqdm(data_list):
        ...
        if np.isnan(test_score_sum[0]):
            print(f"{machine_id} NAN")
            print(test_score.tolist())
        t, th, predict = bf_search(test_score_sum, y_test,
                            start=bf_search_min,
                            end=bf_search_max,
                            step_num=(bf_search_max-bf_search_min)//bf_search_step_size,
                            display_freq=1,
...
if __name__ == '__main__':
    ...
    # bf_results = pd.read_csv(exp_dir/f'evaluation_result/bf_machine_best_f1_d.csv')
    # cluster_config = json.load(open(cluster_json_path))
    # cluster_config.sort(key=lambda x:x['label'])
    # machine_f1_dict = {}
    # for index, row in bf_results.iterrows():
