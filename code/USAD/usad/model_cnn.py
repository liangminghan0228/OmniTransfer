...
class Encoder(nn.Module):
    ...
    def __init__(self, input_dims: int, z_dims: int, nn_size: Sequence[int] = None, freeze_index_list: List =[]):
        ...
        self._net = nn.Sequential(
            nn.Conv2d(in_channels=1, out_channels=channel_base, kernel_size=(7, 7), stride=(Stride_t, Stride_f), padding=(Padding, Padding)),
            nn.ELU(),
            nn.Conv2d(in_channels=channel_base, out_channels=channel_base*2, kernel_size=(7, 7), stride=(Stride_t, Stride_f), padding=(Padding, Padding)),
            nn.ELU(),
            nn.Conv2d(in_channels=channel_base*2, out_channels=channel_base*4, kernel_size=(7, 7), stride=(Stride_t, Stride_f), padding=(Padding, Padding)),
        )
...
class Decoder(nn.Module):
    def __init__(self, z_dims: int, input_dims: int, nn_size: Sequence[int] = None, freeze_index_list: List =[]):
        ...
        if dataset_type == 'data1':
            ...
            out_pad_list = []
        self._net = nn.Sequential(
            nn.ConvTranspose2d(in_channels=channel_base*4, out_channels=channel_base*2, kernel_size=(7, 7), stride=(Stride_t, Stride_f), padding=(Padding, Padding), output_padding=(0, out_pad_list[0])),
            nn.ELU(),
            nn.ConvTranspose2d(in_channels=channel_base*2, out_channels=channel_base, kernel_size=(7, 7), stride=(Stride_t, Stride_f), padding=(Padding, Padding), output_padding=(1, out_pad_list[1])),
            nn.ELU(),
            nn.ConvTranspose2d(in_channels=channel_base, out_channels=1, kernel_size=(7, 7), stride=(Stride_t, Stride_f), padding=(Padding, Padding), output_padding=(1, out_pad_list[2])),
        )
