...
class Encoder(nn.Module):
    ...
    def __init__(self, input_dims: int, z_dims: int, nn_size: Sequence[int] = None, freeze_index_list: List =[]):
        ...
        self.linear2 = nn.Linear(input_dims // 2, input_dims // 4)
        self.linear3 = nn.Linear(input_dims // 4, z_dims)
        self.act = act
    def forward(self, w: torch.Tensor) -> torch.Tensor:
        # z = self._net(w)
        # print(f"encoder z:{z.shape}")
        x1 = self.act(self.linear1(w))
        x2 = self.act(self.linear2(x1))
        z = self.linear3(x2)
        return z,x1,x2
...
class USAD:
    ...
    def valid_log(self, valid_data, model_save_dir, log_file, epoch):
        for w in valid_data:
            ...
            w = w.unsqueeze(dim=1)
            z,_,_ = self._shared_encoder(w)
            w_G = self._decoder_G(z)
            w_G_D = self._decoder_D(self._shared_encoder(w_G)[0])
    ...
    def predict(self, values):
        ...
        with torch.no_grad():
            for w in test_sliding_window:
                ...
                z,x1,x2 = self._shared_encoder(w)
                w_G = self._decoder_G(z)
                w_G_D = self._decoder_D(self._shared_encoder(w_G)[0])
                ...
                x1 = x1.view(-1, self._window_size//2, self._x_dims).cpu().numpy()
                x2 = x2.view(-1, self._window_size//4, self._x_dims).cpu().numpy()
            ...
            return collect_scores_d, collect_scores_g, w_G_list, w_G_D_list, x1_list, x2_list, z_list
