...
def plot(cluster_index, data_idx, res_type):
    ...
    recon_GD = np.load(score_dir/f'{data_idx}/recon_G_D.npy')
    ...
    train_data_item, test_data_item = get_data_by_index(dataset_type, data_idx, preprocess_days)
    train_value, value_list = preprocess_meanstd(train_data_item.T, test_data_item[:, -(len(test_score)):].T)
    # train_value, value_list = preprocess_minmax(train_data_item.T, test_data_item[:, -(len(test_score)):].T)
    ...
    for offline_data in offline_data_list:
        # x_train, _ = preprocess_minmax(offline_data.T, offline_data.T)
    ...
    for row in range(index_num):
        ...
        if res_type == 'd':
            ax.plot(range(label_list.shape[0]), recon_GD[:, row], color='blue', linewidth=2)
        elif res_type == 'g':
            ax.plot(range(label_list.shape[0]), recon_G[:, row], color='orange', linewidth=2)
