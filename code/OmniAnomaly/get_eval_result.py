...
def get_data(data_idx):
    ...
    # test_score_name = "weighted_test_score"
...
def get_bf_by_cluster_threhold(calc_latency):
    ...
    for cluster in clusters:
        ...
        res_machine_path = exp_dir/ f'evaluation_result/best_f1_result/{cluster["label"]}/{res_prefix}_best_f1_machine.csv'
    ...
    best_df.to_csv(exp_dir/f'evaluation_result/{res_prefix}_best_f1.csv', index=False)
...
def read_bf_center_result(calc_latency):
    ...
    print(f"center result get_bf f1: {round(f1, 4)} p:{round(p, 4)} r:{round(r, 4)}", file=res_file)
...
if __name__ == '__main__':
    ...
    bf_results = pd.read_csv(exp_dir/f'evaluation_result/bf_machine_best_f1.csv')
