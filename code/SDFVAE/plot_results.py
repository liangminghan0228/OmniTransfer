...
def init():
    if (exp_dir/ f'{prefix}evaluation_result/{bf_res_prefix}machine_best_f1.csv').exists():
        bf_df = pd.read_csv(exp_dir/ f'{prefix}evaluation_result/{bf_res_prefix}machine_best_f1.csv')
    ...
    if (exp_dir/f"{prefix}evaluation_result/pot_result/best_param.csv").exists():
        pot_df = pd.read_csv(exp_dir/f"{prefix}evaluation_result/pot_result/best_param.csv")
        ...
        for cluster_index, q, level in zip(cluster_list, q_list, level_list):
            best_cluster_pot_df = pd.read_csv(exp_dir/f"{prefix}evaluation_result/pot_result/{cluster_index}/{q}_{level}_pot.csv")
...
def plot(cluster_index, data_idx):
    ...
    test_score = np.load(score_dir / f'{data_idx}/{prefix}test_score.npy')
    ...
    tp_fp_res = np.load(exp_dir/f'{prefix}evaluation_result/{bf_res_prefix}tp_fp_res.npy')
    ...
    train_value, value_list = preprocess(train_data_item.T, test_data_item[:, -(len(test_score)):].T)
    ...
    for row in range(index_num):
        ...
        for preprocess_offline_data in preprocess_offline_data_list:
            if dataset_type=='data1':
                ax.plot(range(288*4-59,288*5-59),preprocess_offline_data[:,row])
            elif dataset_type=='data2':
                ax.plot(range(96*6-59,96*7-59),preprocess_offline_data[:,row])
            else:
                break
...
if __name__ == '__main__':
    ...
    Parallel(n_jobs=plot_job)(delayed(plot)(cluster_index, data_index) for cluster_index, data_index in tqdm(cluster_data_zip))
    # prefix = 'weighted_'
    # init()
    # pic_dir = exp_dir / f'weighted_pic'
