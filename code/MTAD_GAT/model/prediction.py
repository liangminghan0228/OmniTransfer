...
class Predictor:
    ...
    def predict_anomalies(self, train, test, true_anomalies, load_scores=False, save_output=True,
        ...
        e_eval = epsilon_eval(test_anomaly_scores[-len(true_anomalies):], test_anomaly_scores[-len(true_anomalies):], true_anomalies, reg_level=self.reg_level)
        ...
        # e_eval = {}
        p_eval = {}
        # print(test_anomaly_scores.shape)
        if true_anomalies is not None:
            bf_eval = bf_search(test_anomaly_scores[-len(true_anomalies):], true_anomalies, start=0.01, end=2, step_num=100, verbose=False)
        else:
            bf_eval = {}
        ...
        if save_output:
            ...
            test_pred_df["A_True_Global"] = np.concatenate([np.zeros(len(test_pred_df)-len(true_anomalies)),true_anomalies])
