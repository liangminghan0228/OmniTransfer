...
class Trainer:
    ...
    def freeze_layers(self, freeze_layer: str = "freeze_rnn"):
        if "freeze_rnn" == freeze_layer:
            freeze_a_layer(self.model.gru)
            freeze_a_layer(self.model.recon_model.decoder.rnn)

    def unfreeze_layers(self):
        unfreeze_a_layer(self.model.gru)
        unfreeze_a_layer(self.model.recon_model.decoder.rnn)

    def init_layers(self, init_layer: str = 'dense'):
        if init_layer=='dense':
            self.model.feature_gat.lin.reset_parameters()
            self.model.temporal_gat.lin.reset_parameters()

            forecasting_model_layers_len = len(self.model.forecasting_model.layers)
            for i in range(forecasting_model_layers_len):
                self.model.forecasting_model.layers[i].reset_parameters()

            self.model.recon_model.fc.reset_parameters()